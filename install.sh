...
echo $SERVER_PORT
echo "请选择服务器加密方式: "
select method in "chacha20-ietf-poly1305" "aes-256-gcm" "chacha20" "rc4-md5" "salsa20" "aes-256-cfb";do
  case $method in
    chacha20-ietf-poly1305 )SERVER_METHOD="chacha20-ietf-poly1305";break;;
    aes-256-gcm )SERVER_METHOD="aes-256-gcm";break;;
...
read -p "请输入插件名: " SERVER_PLUGIN
echo $SERVER_PLUGIN

read -p "请输入插件参数" SERVER_PLUGIN_OPT
echo $SERVER_PLUGIN_OPT
...
echo "密码："$SERVER_PASS
echo "插件"$SERVER_PLUGIN
echo -e "插件参数"$SERVER_PLUGIN_OPT${end}
echo "是否继续？"
...
sed -i "s|{plugin-name}|$SERVER_PLUGIN|g" conf.d/shadowsocks.conf
sed -i "s|{plugin-opts}|$SERVER_PLUGIN_OPT|g" conf.d/shadowsocks.conf
